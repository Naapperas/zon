...
class PodNumber(Pod):
    def validate(self, data):
        if not isinstance(data, int) and not isinstance(data, float):
            self._add_error(ValidationError("Expected number, got {}".format(type(data))))
            return False
        return True
...
class PodInteger(Pod):
    def validate(self, data):
        if not isinstance(data, int):
            self._add_error(ValidationError("Expected integer, got {}".format(type(data))))
            return False
        return True
...
class PodFloat(Pod):
    def validate(self, data):
        if not isinstance(data, float):
            self._add_error(ValidationError("Expected float, got {}".format(type(data))))
            return False
        return True
...
class PodBoolean(Pod):
    def validate(self, data):
        if not isinstance(data, bool):
            self._add_error(ValidationError("Expected boolean, got {}".format(type(data))))
            return False
        return True
...
class PodString(Pod):
    def validate(self, data):
        if not isinstance(data, str):
            self._add_error(ValidationError("Expected string, got {}".format(type(data))))
            return False
        return True
    ...
    def validate(self, data):
        ...
        for i, element in enumerate(data):
            if not self.element_type.validate(element):
                self._add_error(ValidationError("Element {} of list failed validation: {}".format(i, element)))
...
class PodObject(Pod):
    ...
    def validate(self, data):
        if not isinstance(data, dict):
            self._add_error(ValidationError("Expected object, got {}".format(type(data))))
            return False
        
        error = False
        for key, pod in self.properties.items():
            if not pod.validate(data.get(key)):
                self._add_error(ValidationError("Property {} failed validation: {}".format(key, data.get(key))))
                error = True

        if error:
            for pod in self.properties.values():
                for error in pod.errors:
                    self._add_error(ValidationError(f"Error validating properties: {error}"))
